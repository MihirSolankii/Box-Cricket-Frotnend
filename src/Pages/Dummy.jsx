...
function ScoreCard() {
    ...
    useEffect(() => {
        const init = async () => {
            ...
            try {
                ...
                // Fetch Settlement Members (for dropdown)
                const membersRes = await axios.get(`${routes.getSettlement}/${settlementIdRef.current}`, {
                    headers: { Authorization: `Bearer ${tokenRef.current}` }
                });

                // Extract nicknames correctly
                if (membersRes.data.settlement?.participants) {
                    const nicknames = membersRes.data.settlement.participants.map(p => p.nickname);
    ...
    useEffect(() => {
        ...
        socket.on("updateMatch", (data) => {
            setMatch(data);
            setLoading(false);
            setMaxOversInput(data.maxOvers); // Sync inputs
        });
        ...
        socket.on("notification", (data) => {
            ...
            switch (data.type) {
                ...
                case "error":
                    ...
                    break;
                default:
                    toast(data.message); // Default info toast
            }
        });
    ...
    const handleWicketSubmit = () => {
        ...
        setWhoDismissed("striker");
    };

    const handleRecordBall = (runs, isWide, isNoBall) => {
        socket.emit("recordBall", {
            matchId,
            ballData: { runs, isWide, isNoBall, isWicket: false }
        });
    };
    ...
    if (loading) return (
        <div className="min-h-screen bg-background flex items-center justify-center text-primary">
            <Activity className="animate-spin mr-2" /> Loading Scoreboard...
        </div>
    );

    if (!match) return <div className="min-h-screen bg-background flex items-center justify-center">Waiting for match...</div>;
    ...
    // Logic for 2nd Innings Check
    const isSecondInnings = (match.target && match.target > 0) ||
        (match.currentInnings === "A" && match.teamB.balls > 0) ||
        (match.currentInnings === "B" && match.teamA.balls > 0);
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="flex justify-between items-center">
                    ...
                    {isAdmin ? (
                        ...
                        <span className="flex items-center gap-2 px-3 py-1 bg-muted text-muted-foreground rounded-full text-xs font-bold border border-border">
                            <Activity size={12} /> SPECTATOR
                        </span>
                    )}
                </div>
                ...
                <div className="card-turf bg-card p-6 border border-border relative overflow-hidden">
                    <div className="flex justify-between items-start mb-2 relative z-10">
                        ...
                        {isSecondInnings && (
                            <div className="text-xs bg-muted px-2 py-1 rounded text-foreground font-mono">
                                Target: {match.target}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="mt-6 flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
                            ...
                            {match.currentOverHistory?.map((ball, i) => (
                                <div key={i} className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full text-xs font-bold shadow-sm border
                                    ${ball.includes('W') ? 'bg-destructive text-destructive-foreground border-destructive' :
                                        ball.includes('4') || ball.includes('6') ? 'bg-success text-success-foreground border-success' :
                ...
                {isAdmin && (
                    <div className="grid grid-cols-2 gap-4">
                        <button
                            ...
                            className="flex items-center justify-center gap-2 p-3 bg-card border border-border rounded-xl font-bold text-sm shadow-sm hover:bg-secondary transition"
                        >
                            <UserPlus size={18} className="text-primary" />
                            Add Squad
                        </button>
                        <button
                            onClick={() => setShowSetPlayers(true)}
                            className="flex items-center justify-center gap-2 p-3 bg-card border border-border rounded-xl font-bold text-sm shadow-sm hover:bg-secondary transition"
                        >
                ...
                {isAdmin && match.matchStatus !== 'completed' && (
                    <div className="bg-card p-5 rounded-xl border border-border shadow-sm">
                        <h3 className="text-xs text-muted-foreground font-bold uppercase tracking-wider mb-4">Scoring</h3>

                        <div className="grid grid-cols-6 gap-2 mb-4">
                            {[0, 1, 2, 3, 4, 6].map((run) => (
                                <button key={run} onClick={() => handleRecordBall(run, false, false)}
                                    className="aspect-square rounded-lg bg-secondary hover:bg-primary hover:text-primary-foreground font-bold text-lg text-foreground transition border border-border shadow-sm">
                                    {run}
                                </button>
                            ))}
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="md:col-span-2 bg-card rounded-xl p-5 border border-border shadow-sm">
                        <h3 className="text-muted-foreground text-xs font-bold uppercase mb-4 flex items-center gap-2">
                            <CircleDot size={16} className="text-primary" /> On Crease
                        </h3>
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="text-muted-foreground text-xs uppercase tracking-wider text-right border-b border-border">
                                    ...
                                    <th className="py-2 hidden md:table-cell">4s</th><th className="py-2 hidden md:table-cell">6s</th>
                    ...
                    <div className="bg-card rounded-xl p-5 border border-border shadow-sm flex flex-col justify-between">
                        <h3 className="text-muted-foreground text-xs font-bold uppercase mb-4 flex items-center gap-2">
                            <Activity size={16} className="text-blue-500" /> Bowler
                        </h3>
                        {match.bowler ? (
                            <div className="text-center flex-1 flex flex-col justify-center">
                                <div className="w-16 h-16 bg-secondary rounded-full mx-auto mb-3 flex items-center justify-center text-xl font-bold text-foreground border border-border">
                                    {match.bowler.charAt(0)}
                                </div>
                                ...
                                <div className="mt-4 grid grid-cols-3 gap-2 text-center text-sm">
                                    <div className="bg-secondary p-2 rounded border border-border">
                                        <div className="text-[10px] text-muted-foreground uppercase">Ov</div>
                                        <div className="font-bold text-primary">{match.bowlers?.[match.bowler]?.overs}</div>
                                    </div>
                                    <div className="bg-secondary p-2 rounded border border-border">
                                        <div className="text-[10px] text-muted-foreground uppercase">Run</div>
                                        <div className="font-bold text-foreground">{match.bowlers?.[match.bowler]?.runs}</div>
                                    </div>
                                    <div className="bg-secondary p-2 rounded border border-border">
                                        <div className="text-[10px] text-muted-foreground uppercase">Wkt</div>
                                        <div className="font-bold text-destructive">{match.bowlers?.[match.bowler]?.wickets}</div>
                                    </div>
                                </div>
                            </div>
                        ) : <div className="flex-1 flex items-center justify-center text-muted-foreground italic text-sm">No bowler active</div>}
                    </div>
                </div>
            </div>

            {/* --- MODAL: MATCH SETTINGS (Toss / Overs) --- */}
            <AnimatePresence>
                {showSettingsModal && (
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                            ...
                            <div className="mb-4">
                                ...
                                <input
                                    type="number"
                                    value={maxOversInput}
                                    onChange={(e) => setMaxOversInput(e.target.value)}
                                    className="w-full bg-secondary border border-border p-3 rounded-lg text-foreground outline-none focus:ring-2 ring-primary"
                                />
                            </div>
                            ...
                            <div className="mb-6">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <button
                                        onClick={() => setBattingFirstInput("A")}
                                        className={`p-3 rounded-lg border font-bold text-sm transition ${battingFirstInput === "A" ? "bg-primary text-white border-primary" : "bg-secondary text-muted-foreground"}`}
                                    >
                                        {match.teamA.name}
                                    </button>
                                    <button
                                        onClick={() => setBattingFirstInput("B")}
                                        className={`p-3 rounded-lg border font-bold text-sm transition ${battingFirstInput === "B" ? "bg-primary text-white border-primary" : "bg-secondary text-muted-foreground"}`}
                                    >
                                        {match.teamB.name}
                                    </button>
                                </div>
